...
void essential :: computePose(){
  ...
  //refrence camera projection matrix
  P0 = cv::Mat::eye(3, 4, CV_64FC1);
  //four possible solution for the next camera projection matrix.
  P1 = cv::Mat(3, 4, CV_64FC1, Scalar(0)); 
  P2 = cv::Mat(3, 4, CV_64FC1, Scalar(0));
  P3 = cv::Mat(3, 4, CV_64FC1, Scalar(0)); 
  P4 = cv::Mat(3, 4, CV_64FC1, Scalar(0));
  ...
  P0 = K*P0; 
  P1 = K*P1; P2 = K*P2; P3 = K*P3; P4 = K*P4;
}
...
void essential :: check_chirality(Mat Xn1, Mat Xn2, Mat Xn3, Mat Xn4){
  ...
  Mat chiral1 = sign( ax1.row(2) ).mul( sign( Xn1.t().row(3) ) ) + 
                sign( bx1.row(2) ).mul( sign( Xn1.t().row(3) ) );
  
  Mat chiral2 = sign( ax2.row(2) ).mul( sign( Xn2.t().row(3) ) ) + 
                sign( bx2.row(2) ).mul( sign( Xn2.t().row(3) ) );

  Mat chiral3 = sign( ax3.row(2) ).mul( sign( Xn3.t().row(3) ) ) + 
                sign( bx3.row(2) ).mul( sign( Xn3.t().row(3) ) );

  Mat chiral4 = sign( ax4.row(2) ).mul( sign( Xn4.t().row(3) ) ) + 
                sign( bx4.row(2) ).mul( sign( Xn4.t().row(3) ) );

  Mat chiral_sum(1, 4, CV_64F);
  Scalar sc1 = sum(chiral1);
  Scalar sc2 = sum(chiral2);
  Scalar sc3 = sum(chiral3);
  Scalar sc4 = sum(chiral4);

  chiral_sum.at<double>(0, 0) = (double)sc1(0);
  chiral_sum.at<double>(0, 1) = (double)sc2(0);
  chiral_sum.at<double>(0, 2) = (double)sc3(0);
  chiral_sum.at<double>(0, 3) = (double)sc4(0);
